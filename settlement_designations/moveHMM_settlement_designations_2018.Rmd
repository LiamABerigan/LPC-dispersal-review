...
Only concerned with birds from release year 2018
```{r}
lpc_gps %>%
...
18 ("2018.172603") initial delay threw off the algorithm

40 ("2018.172630") the second movement mode is only favorable in a few select, stopover scenarios
44 ("2018.172637") two transitions are likely
45 ("2018.172640") second movement mode is never likely
45 ("2018.172642") two transitions are likely



```{r}
ltraj_list[[18]] <- NULL; modpartltraj_objects[[18]] <- NULL
ltraj_list[[40]] <- NULL; modpartltraj_objects[[40]] <- NULL
ltraj_list[[44]] <- NULL; modpartltraj_objects[[44]] <- NULL
ltraj_list[[45]] <- NULL; modpartltraj_objects[[45]] <- NULL
ltraj_list[[45]] <- NULL; modpartltraj_objects[[45]] <- NULL
```
...
Removing all points from certain trajectories that broke because they were all dispersal points
```{r}
reg_groups %>%
  filter(Haukos_SatID != "2018.172593") %>%
  filter(Haukos_SatID != "2018.172596") %>%
  filter(Haukos_SatID != "2018.172608") %>%
  filter(Haukos_SatID != "2018.172609") %>%
  filter(Haukos_SatID != "2018.172611") %>%
  filter(Haukos_SatID != "2018.172621") ->
  reg_groups
```
...
reg_groups %>%
  ...
  st_write(dsn = here_file("Projects", "Dispersal_corridors", "Dispersal_stats_redo"), layer = "settlement_locations_2018", driver = "ESRI Shapefile")
```
...
reg_groups %>%
  filter(reg_group == 0) %>%
  st_as_sf(coords = c("x", "y"), crs = 5070) %>%
  st_write(dsn = here_file("Projects", "Dispersal_corridors", "Dispersal_stats_redo"), layer = "dispersal_locations_2018", driver = "ESRI Shapefile")
```